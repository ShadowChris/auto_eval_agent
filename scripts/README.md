...
每条 JSONL 会先输出 `id`、`index`/`序号`、`query`、`context`、
`answer`、任务时间和 `video_path` 等标准关键字段，再按原表列顺序追加
所有其他原始列。参与过字段映射的辅助列（例如 `回复内容`、`开始时间节点`、
`文件路径`、`原文件名`）也会保留；原列名与标准字段同名时不重复输出，
并直接使用原列值。输入表存在可自动生成的同名标准列 `context`、`answer`、
`task_start_time`、`task_end_time` 或 `video_path` 时，命令行会打印警告，
同时停止该字段的自动拼接或别名映射；不存在时才沿用下面的自动生成规则。
...
- 输入表存在 `video_path` 列：所有行都直接使用该列，不再读取或拼接
  `文件路径`、`原文件名`；某行 `video_path` 为空时会输出字符串
  `__missing_video__` 作为占位值并记录缺失警告，不会回退使用其他路径列。
