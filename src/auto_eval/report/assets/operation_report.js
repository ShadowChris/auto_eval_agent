...
(function (global) {
  ...
  function mount(root, initialPayload) {
    ...
    function render() {
      ...
      root.innerHTML = `
        <div class="or-heading"><div><div class="or-name">${isCompare ? "垂域对比分析报告" : "垂域评估统计报告"}</div><div class="or-muted">${subtitle}</div></div>
    ...
    function filteredCases() {
      ...
      return pairs.filter(row => {
        ...
        return !keyword || [right.item_id, right.index, right.query, right.answer_text, right.problem_solved_reason, right.rationale, left?.problem_solved_reason, left?.rationale].map(text).join(" ").toLocaleLowerCase().includes(keyword);
      });
    }
    function detail(row, group) {
      const fields = [
        ["垂域", row.category_display],
        ["context", row.context],
        ["answer_text", row.answer_text],
        ["评价原因", row.problem_solved_reason],
        ["全部问题类型", row.issue_types.join("；") || "无"],
        ["原始问题记录", row.answer_issues],
        ["需人工复核", row.needs_review],
        ["复核原因", row.review_reason],
        ["事实冲突", row.factual_conflict],
        ["冲突内容", row.factual_conflict_detail],
        ["挂卡数量", row.card_count],
        ["Superlink数量", row.superlink_count],
        ["识别结论", row.rationale],
        ["评估耗时（秒）", row.latency_s],
      ];
      return `<div><strong>${esc(group)} · ${verdict(row)}</strong>${fields.filter(([, v]) => v !== "" && v != null).map(([k, v]) => `<p><span class="or-muted">${k}</span><br>${esc(text(v))}</p>`).join("")}</div>`;
    }
    function snippet(row) {
      const source = text(row.problem_solved_reason || row.rationale || "");
      return `${source.slice(0, 110)}${source.length > 110 ? "…" : ""}`;
    }
    function renderCases() {
      ...
      const heads = ["题号", "query", ...(comparison() ? ["对照组", "实验组", "问题变化"] : ["判定", "理由摘要"]), "详情"];
      ...
      for (const row of rows.slice((state.page - 1) * state.size, state.page * state.size)) {
        ...
        body += `<tr><td class="or-id">${esc(right.item_id === "" || right.item_id == null ? right.index : right.item_id)}</td><td class="or-query">${esc(right.query)}</td>${comparison()
          ? `<td>${verdict(left)}</td><td>${verdict(right)}</td><td>${esc(change)}</td>`
          : `<td>${verdict(right)}</td><td class="or-snippet">${esc(snippet(right))}</td>`
