...
- 特殊情况（多轮未闭环）：用户下发指令后模型没有直接执行需求，而是额外发起反问。记为「多轮对话造成需求未闭环」，而不是「需求未闭环」。注意：是否解决用户问题仍由下方 problem_solved 规则判断，这里仅记录存在的问题，仅在未执行未闭环的情况下才可能属于「多轮对话造成需求未闭环」。
...
难区分的 answer_issues：
1. 多轮请求未闭环
    > 判定细则：用户下发指令后模型没有直接执行需求，而是额外发起反问。需要判断是真的客观原因造成无法执行还是用户已经有明确意图但产品无法识别出来。如果是产品无法识别，是产品能力问题 → 判定 nok。
    > 示例：
        - 用户明确要求导航至指定地点，应答返回地点列表交由用户手动选择，列表内已存在完全匹配目标地点 → nok。
        - 用户明确要求导航至指定地点，应答返回的地点中没有完全匹配目标地点，要求用户澄清想去的位置，属于客观原因，但需在 answer_issues 标注「多轮对话造成需求未闭环」 → ok。
2. 正文卡不一致，回答内容前后自相矛盾
