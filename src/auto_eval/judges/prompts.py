...
纪律：
1. 竞品答案只是另一支/另一产品给出的答案，不一定正确、更不代表标准答案——本字段只记录「是否事实冲突」，绝不判断谁对谁错；只有出现**事实冲突**才标记，绝不能因为"产品回答与竞品答案不一致"就判错或扣分；
2. 竞品答案不是评分标准：不得让它影响 problem_solved / answer_issues / card_suitability 等其他字段的判定；产品回答即使与竞品答案不同，只要自身正确、有据可依，就应按实际表现评判；
3. 省略该字段即视为「无冲突」。

【人工复核 needs_review】
出现用户query和画面中用户query严重不符、产品answer_text和画面中的严重不符、画面模糊、内容被遮挡、回答覆盖不完整、跨帧无法可靠去重、卡片类型或蓝字归属不确定时，needs_review=true 并说明原因。
如果出现类似手机闹钟打断了操作或其他外部因素导致无法判断时，也应标记 needs_review=true 并说明原因。
