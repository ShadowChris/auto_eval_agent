...
<analysis>
1. 【原始目标】明确 query 的原始动作目标、完成边界、任务形态、有效时间窗、条件与生效目标；不要把执行过程中的查询、候选列表或内容准备误当成原始动作已经完成。
2. 【视觉指引扫描】按时间顺序检查全部关键帧，逐字记录登录、授权、验证、补充、选择、确认、超时或终止等清晰可辨文字并注明帧序号。普通“登录”入口、“查看”按钮、“手动操作中”状态、接管或停止控件、加载动画和“已完成”都不是用户指引；但录屏仍停留在登录、授权或身份验证页面时，“超时/终止”可以与该页面及最终反馈共同证明任务因等待用户而结束。没有则明确写“未发现”。当前画面只展示部分列表或没有出现目标对象时，不得据此推断对象不存在。
3. 【文本指引扫描】通读完整 agent 自述，逐字摘录“无法、缺少、未提供、请提供、请选择、是否、要不要、请确认”等与继续任务直接相关的完整原句；没有则明确写“未发现”，不得意译或补写原文中没有的询问。“无法继续”“请自行操作”“相关工具已返回结果”等泛化终止语只有在同时说明具体能力限制、缺失条件或用户下一步动作时，才算阻塞证据。
4. 【来源分类】标明上述文字属于界面用户指引、任务执行面板的步骤和流程状态，还是 agent 面向用户的自然语言回复；禁止把任务面板的步骤、“已完成”或“超时，任务终止”当成 agent 回复。
5. 【逐目标状态】将每个生效目标归入：已完成、等待用户、缺少前置条件、静默停滞、临时异常、可归责错误或证据不足，并引用直接证据。
6. 【阻塞资格表】若考虑等待用户或缺少前置条件，必须逐项写明：A. 用户必须补充的信息或完成的具体动作；B. 为什么这是继续原任务不可替代的条件，而不是让用户选择 agent 的搜索、导航、工具或补救策略；C. 最后有效状态中该阻塞是否仍未解除；D. 阻塞前是否已有足以独立导致失败的可见错误。只有 A/B/C 成立且 D 为否，才允许判 no_support。“需要我继续到设置中查找吗”“换关键词、换入口还是上报”等都是恢复策略询问，不是必要澄清；专家经验确认能力存在时，找不到入口也不能改判等待用户。复杂任务若在当前步骤进入真实必要的澄清并停止整条串行任务流，后续尚未开始的目标属于中断后果，不单独判遗漏。
7. 【时序与强制映射】录屏停在登录、授权或身份验证页面，同时出现明确用户接管指引或“本次任务超时/终止”反馈时，可以归为缺少前置条件。只有普通登录入口或登录页面、三路证据均无澄清提示、agent 最终回答为空、没有等待反馈且流程静默停止时，才归为三方应用跳转中断。用户实际答复、选择、确认、完成登录授权，或后续画面进入应用内容、搜索结果、商品详情等阻塞后的执行页面时，旧阻塞立即解除；解除后必须重新评价剩余目标，即使后续执行错误或不完整。仅返回桌面或助手界面不算解除。
8. 【结果证据专项】若目标要求播放，结果卡、详情卡、播放按钮和流程“已完成”都不证明已经播放；必须检查播放器、进度、暂停按钮或同等直接播放状态。
9. 【最终结论】汇总 correctness、issue_types 与 is_low_level。标记回复与界面不一致前，必须引用冲突的具体 agent 自然语言回复；不要让 issue type 反向覆盖已经确认的根因。
</analysis>
