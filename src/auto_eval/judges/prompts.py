...
3. 用户需求形成闭环即可判定为 ok。
问题特例：
- 特例「思考暴露」、「挂卡/资源缺失」、「结果重复」、「操作冗余」、「结构/总结问题」：即便存在这几类问题，只要最终完成用户需求闭环即可标记 ok，但该缺陷仍必须记录在 answer_issues 字段中。
- 特例「需求未闭环」：需要慎重按合理性判断，首先判断产品对用户需求理解是否正确，然后对话钩子设计合理（可引导下一轮推进），最后本轮内容本身无重大缺陷。三个都满足的情况下合理的对话钩子记录为ok，同时needs_review设置为true，review原因记录“需要确认导致需求未闭环”；不合理的对话钩子直接记录nok，answer_issues记需求未闭环。
    - 例子1：用户要求删除照片，产品需要询问是否确定删除。属于ok，原因是“删除”操作是危险操作，要求用户二次确认是合理情况
    - 例子2：用户要求导航到最近的肯德基餐厅，产品搜索后列出了所有肯德基餐厅并询问去第几个。属于nok，原因是用户已经要求去“最近的”，除非有距离完全一样的，否则应该直接导航而不是询问去第几个。
