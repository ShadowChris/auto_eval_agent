...
- 用户 query：{{ question }}
- 可信 context：{{ context }}
  + context 中可能包含了用户明确指定的条件（如时间、地点、风格、版本、集数、数量等），如果用户query中没有新的明确指明的条件，则以 context 中的明确地址为主。地址、时间等条件的优先级是：用户query中明确说明的 > 过去用户query中明确说明的 > context中过去评测产品回答中的（须先判断过去产品的意图理解是否正确，如果错误则不可用） > context中自动记录的用户位置时间。
...
用户意图的条件优先级以【背景信息】context 中的定义为准（用户query明确说明的 > 过去用户query明确说明的 > context中过去产品回答的 > context中自动记录的用户位置时间），此处不重复展开。在此之上还需注意两条判断要点：其一，若上一轮中产品回答的内容与用户意图不符，注意本轮是否纠正了上一轮的偏差；其二，导航时用户要求去任何地方都是合理的，无需假设用户想要本地导航。
