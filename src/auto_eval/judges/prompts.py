...
判定需同时满足：
1. `answer_issues` 不存在严重缺陷，允许存在不影响核心结果的轻微瑕疵；
2. 文本内容与可视化组件共同输出正确、完整的应答结果；
3. 用户核心需求达成闭环，即可判定为 ok。

**问题特例：**
- 特例1：对于「思考暴露」「挂卡/资源缺失」「结果重复」「操作冗余」「结构/总结问题」。回答即便存在以上问题，只要最终完成用户需求闭环，`problem_solved` 仍标记为 ok，但对应缺陷必须如实写入 `answer_issues`。
- 特例2：「需求未闭环」场景判定。需依次校验三项条件：产品对用户需求理解无误、对话钩子设计合理，可用于引导下一轮交互、本轮输出内容无重大缺陷。三项全部满足时，属于合理对话钩子：`problem_solved` 填 ok，同时置 `needs_review=true`，复核原因填写“需要确认导致需求未闭环”。
若对话钩子设计不合理，则 `problem_solved` 填 nok，并将「需求未闭环」写入 `answer_issues`。
> 示例：
> 1. 用户请求删除照片，助手询问确认是否删除 → ok。删除属于高危操作，二次确认属于合理钩子。（同类高危场景：付费打车、下单购买等资金风险操作）
> 2. 用户要求导航至某地点，无法精准定位目标，返回候选地点交由用户选择 → ok。用户原始请求信息模糊，二次确认属于合理钩子。
> 3. 用户要求导航至某地点，返回完全无关的地点结果 → nok。未输出有效可用信息，需求无法闭环。
> 4. 用户请求清晰明确，存在同名多个地点，无客观歧义却仍询问用户选择 → nok。输入条件充足，应当直接完成服务闭环，不应额外确认。
> 5. 用户要求导航到最近的肯德基，返回全部肯德基列表并询问选择哪一个 → nok。用户明确指定“最近”，除非多个门店距离完全相等，应当直接执行导航，无需询问选择。

> 通用原则：该逻辑可迁移到全部业务场景。**用户请求清晰完备时，仅允许必要的高危类二次确认；用户请求信息模糊不足时，发起二次确认属于合理处理，可判定 ok。**
