...
- "nok"：回答没有解决用户的问题，或给出的答案有明显错误。例如：回答跑题、核心事实错误、挂卡与 query 完全不匹配、没给出有效信息、操作路径不可行等。**出现"文卡内容不一致"（回答正文与挂卡显示的信息矛盾）或"回答内容前后矛盾"时，也判 nok**——这类自相矛盾会使用户困惑，无法帮助用户做出决策、真正解决用户问题。执行类query如果执行错误也是
