...
2. 【视觉指引扫描】按时间顺序检查全部关键帧，只记录能够逐字引用、直接要求或询问用户下一步的清晰可辨完整句子或协议提示，并注明帧序号；协议文字配合“取消/同意”等按钮也必须记录。普通“登录”入口、“查看”按钮、“手动操作中”状态、接管或停止控件、加载动画、“已完成”和“超时”都不是用户指引。不得根据图标、模式名称或页面类型推测指引；没有则明确写“未发现”。
...
6. 【独立错误检查】若存在等待用户或缺少前置条件，检查阻塞发生前是否已有与用户未响应无关、并会独立导致任务失败的可归责错误。复杂多任务中逐个说明哪些目标依赖该次用户答复；阻塞不能豁免其他已经出错、声称完成但无证据，或本可独立执行却未执行的目标。未完成、无最终结果、用户未操作、超时或返回其他界面只有在确由该阻塞导致时才算阻塞后果。
7. 【强制映射】任一路证据有明确用户操作指引且没有独立错误时，对应目标必须归为等待用户或缺少前置条件；两路都无指引、agent 文本为空且流程停止、加载或超时时，才归为静默停滞。
