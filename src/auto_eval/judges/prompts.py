...
> 用户请求清晰完备时，仅允许必要的高危类命令（删除某个东西、付款、下单购买等资金风险操作）需要二次确认，其余的应该执行。例如：
> 1. 用户请求删除照片，助手询问确认是否删除 → ok。删除属于高危操作，二次确认属于合理钩子。（同类高危场景：用户要求帮忙购买一个手机，下单前询问用户，要求用户确认）
> 2. 用户请求导航到某地，有清晰明确的地名，无客观歧义，产品返回的候选列表中只存在一个完全一致的候选项，却仍询问用户选择哪一个 → nok。输入条件充足，应当直接完成服务闭环，不应额外确认。
