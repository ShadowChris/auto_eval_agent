...
2. 【视觉指引扫描】按时间顺序检查全部关键帧，逐字记录登录、授权、验证、补充、选择、确认、超时或终止等清晰可辨文字并注明帧序号。普通“登录”入口、“查看”按钮、“手动操作中”状态、接管或停止控件、加载动画和“已完成”都不是用户指引；但录屏仍停留在登录、授权或身份验证页面时，“超时/终止”可以与该页面及最终反馈共同证明任务因等待用户而结束。没有则明确写“未发现”。
3. 【文本指引扫描】通读完整 agent 自述，逐字摘录“无法、缺少、未提供、请提供、请选择、是否、要不要、请确认”等与继续任务直接相关的完整原句；没有则明确写“未发现”，不得意译或补写原文中没有的询问。
...
6. 【独立错误检查】若存在等待用户或缺少前置条件，检查阻塞前是否已有黑盒可见、与用户未响应无关且足以导致失败的错误。第三方登录场景不得使用日志、内部规划或 skill 调用推断本应打开哪个应用。复杂任务若在当前步骤进入真实必要的澄清并停止整条串行任务流，后续尚未开始的目标属于中断后果，不单独判遗漏；阻塞前已有错误或无证据完成声明仍需评价。
7. 【强制映射】任一路证据有明确用户操作指引且没有可见独立错误时，对应目标必须归为等待用户或缺少前置条件。录屏停在登录、授权或身份验证页面，同时出现明确用户接管指引或“本次任务超时/终止”反馈时，可以归为缺少前置条件；仅有普通登录入口、最终回答为空且没有任何反馈时归为静默停滞或三方应用跳转中断。
