...
- ok：对用户需求理解正确 + 对话钩子设计合理（可引导下一轮推进）+ 本轮内容本身无重大缺陷（允许为后续轮次预留信息而不闭环）；若属于**合理确认**（需求理解正确，受客观限制需要用户补充信息），判定 ok，同时 need_review=true、review_reason="需要多轮确认"，不写入 answer_issues。
- nok：本轮自身做错（答非所问、行为/操作错误、给出错误信息、挂卡明显不相关等）；若属于**不合理确认**（答案本可获取却反问，用户条件已充分仍索要信息），nok，answer_issues 记录「需求未闭环」。
- need_review：信息不足。

### 分支B：最后一轮（含独立单题）
允许使用「需求未闭环」标签，完整闭环校验。
