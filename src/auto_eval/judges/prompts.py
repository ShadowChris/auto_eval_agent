...
4. N/A 判断：哪些维度不适用及理由...
5. 对比锚点：...
6. 主要问题点：选出问题最严重的前3个**不同维度**（从真实性、准确性、完整性、相关性、合规性、有用性、结构性中选择，维度必须不同，不足3个则只列出存在的），再为每个维度写具体问题描述...
</analysis>
{"rubric": { {%- for d in dims -%} {%- if d.sub_dimensions -%} "{{ d.name }}": { {%- for s in d.sub_dimensions -%} "{{ s.name }}": <1-{{ scale }} 或 null>, {% endfor -%} "total": <非null子维度的均值>, "reason": "<该维度为何打这分的简短理由>" }, {%- else -%} "{{ d.name }}": { "total": <1-{{ scale }} 或 null>, "reason": "<该维度为何打这分的简短理由>" }, {%- endif -%} {%- endfor -%} }, "total": <各适用维度均值按weight加权>, "correctness": "right|wrong|partial|unclear", "error_type": "<简短归因标签，无错误填 null>", "rationale": "<一句话总结>", "top_issue_1_dim": "<首要问题维度，从真实性/准确性/完整性/相关性/合规性/有用性/结构性中选；无问题填 N/A>", "top_issue_2_dim": "<次要问题维度，需与top_issue_1_dim不同；不足则填 N/A>", "top_issue_3_dim": "<第三问题维度，需与前两个不同；不足则填 N/A>", "top_issues_desc": "<问题描述，按行列出：top1维度：描述 \\n top2维度：描述 \\n top3维度：描述；无问题填 N/A>"}
"""
)
