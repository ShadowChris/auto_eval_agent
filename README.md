...
界面操作：选择评测模式 → 导入 JSONL（多轮会话可导入 CSV，按 `session_group` 串行、`turn_index` 排序）→ 开始评测，SSE 实时出结果，完成后可导出。模型调用限流、流水线准入、单题超时与裁判在页首「系统设置」中统一管理（所有任务共享一个队列，默认**每秒 10 次模型请求**的令牌桶速率限流 + **最大在途 50** 的并发上限——速率限制每秒发起数、在途上限限制同时进行的请求数以防模型服务商并发限制；另设「等待容量」=「抽帧中 + 已抽帧待评」的总额度，封顶已备好关键帧的内存，流水线总准入 = 最大在途 + 等待容量；新评测与重跑按当前全局裁判执行）；同一组内任一轮评测失败，同组后续轮次会一起标记失败。输入可携带「事实参考答案」（CSV 列名 `事实参考答案` 或英文列头 `reference_answer`，JSONL 字段 `reference_answer`）：它不代表正确答案，仅用于判定新增维度「事实冲突」（产品回答与参考答案在说同一件事且结论完全矛盾时记「是」，其余一律「否」，不判断谁对谁错）；判定为「是」时会附带「冲突内容」列，一句话写明两边各说了什么。结果表按任务输入顺序展示；每行前有多选框可勾选要重跑的题（成功/失败/未评估均可），多轮组勾选任一轮会自动选中该轮到组尾整段重跑（复用前序轮总结），支持一键全选失败项（`POST /api/eval/rerun`）；重跑期间选中条目在「逐题运行进度」中重新排队并实时更新（头部显示「重跑中… k/N」），中途刷新页面会自动重连进度流。
