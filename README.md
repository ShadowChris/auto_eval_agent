...
界面操作：选择评测模式 → 导入 JSONL（多轮会话可导入 CSV，按 `session_group` 串行、`turn_index` 排序）→ 开始评测，SSE 实时出结果，完成后可导出。全局并发上限、单题超时与裁判在页首「系统设置」中统一管理（所有任务共享一个队列，默认并发 10；新评测与重跑按当前全局裁判执行）；同一组内任一轮评测失败，同组后续轮次会一起标记失败。结果表按任务输入顺序展示；每行前有多选框可勾选要重跑的题（成功/失败/未评估均可），多轮组勾选任一轮会自动选中该轮到组尾整段重跑（复用前序轮总结），支持一键全选失败项（`POST /api/eval/rerun`）。
